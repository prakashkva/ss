...
// BOLD: POv2 ROBO Installer START
//ella.doAjaxAddToCart(variant_id, quantity, title, image);
ella.doAjaxAddToCart(variant_id, quantity, title, image, this.closest('form'));
// BOLD: POv2 ROBO Installer END
...
var error_callback = function(xhr, text) {
                    ella.hideLoading();
                    $('.ajax-error-message').text($.parseJSON(xhr.responseText).description);
                    ella.showModal('.ajax-error-modal');
                };
...
var template = '<li class="item" id="cart-item-{ID}"><a href="{URL}" title="{TITLE}" class="product-image"><img src="{IMAGE}" alt="{TITLE}"></a><div class="product-details"><a href="javascript:void(0)" title="Remove This Item" class="btn-remove">X</a><p class="product-name"><a href="{URL}">{TITLE}</a>{PROPERTIES}</p><div class="cart-collateral">{QUANTITY} x <span class="price">{PRICE}</span></div></div></li>';
// BOLD: POv2 ROBO Installer END
...
item = item.replace(/\{ID\}/g, cart.items[i].key);
item = item.replace(/\{PROPERTIES\}/g, cart.items[i].properties_formatted);
// BOLD: POv2 ROBO Installer END
